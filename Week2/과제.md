...
SSH는 Secure Shell Protocol로 다른 컴퓨터와 public network를 통해 통신할 때 public key와 private key의 조합으로 보안상 안전하게 통신할 수 있게 해주는 프로토콜이다. 

SSH 프로토콜을 사용하기 위해서는 client에는 ssh client , server에는 ssh server가 설치되어 있어야한다. 
...
### nginx log 파일 정리하기. (내가 하고 싶어서)

nginx의 log 파일을 default로 두면 로그파일을 읽기도 어렵고, 불필요하게 자원을 차지한다. 따라서 nginx의 log 파일의 설정을 어느 정도 바꿔주는 것이 좋다.

<br/>

일단 nginx를 패키지 매니저를 이용해 설치하고 아무런 설정을 안 해주면 log, error를 server block에서 나눠서 모아주는 것이 아니라, 하나의 파일에 기록해 주므로 가독성이 떨어진다. 따라서 server block에 따라 log, error 파일들이 나눠서 저장되게 해줘야 한다. 

Nginx 설정 파일의 http 블록에 해당 설정들을 추가해 주면 된다. 

```
// /etc/nginx/nginx.conf

access_log off; // 전역으로 생기는 로그들 비활성화
log_not_found off; // 로그 파일을 못찾아도 에러 발생 x
error_log /var/log/nginx/error.log crit; // crit한 에러만 전역에러에 저장 
```

<br/>

그 뒤 각 server block에서 access_log 파일을 어디에 저장할지 설정을 추가해주면 된다. 
